...
Usage:
    uv run caption_dngs_final.py /path/to/root/directory
...
def extract_preview(dng_path: Path) -> Path:
    ...
    if result.stdout:
        ...
        raise ValueError(f"No PreviewImage found")
...
def generate_caption(image_path: Path) -> str:
    ...
    prompt = """Write a long, detailed caption describing this image. Include:
- Subject's gender, approximate age, skin tone, hair color
- Body parts visible (whether clothed or bare)
- Clothing they are wearing (style, color, details)
- Pose, action, or activity
- Objects visible in the scene
- Setting and background details
Be specific about all visible details."""
    ...
    if not caption:
        raise ValueError("Empty caption returned from model")
